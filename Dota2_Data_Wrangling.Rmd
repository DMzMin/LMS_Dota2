...
**hero_names.csv** - originally contained 112 observations over 3 variables. This file will have more variables added based upon defined roles and classes from dota2.gamepedia.com/Role [link](http://dota2.gamepedia.com/Role).
...
##### **match_id**
The 'match_id' variable is an integer number that Devin updated with his own numbers from 0 to 49,999, which we also find in 'players' data frame and are consistent accross all files. As discussed with Devin (see here https://www.kaggle.com/louisms/discussion [link](https://www.kaggle.com/louisms/discussion) see 'A quick look at Dota 2 dataset') he did this to save space in his files since the orginal 'match_id' values from Dota 2 servers are much longer.  This variable will be key to linking the 'match' and 'players' data frames together.
...
The variables starting with 'unit_order_' (columns 40 - 73 in 'players' data frame) represent the number of physical mouse clicks a player did to perfrom that action.  The usefulness of these variables are not immediately obvious and will initially be removed from the working data set going forward.  If their value is deemed worthy at a later point these will be reintroduced as appropriate.

```{r remove unit order columns}
# remove unit order columns from the data set
PlayersShort <- players[c(1:39)]
```

Lets look at the histograms of some of these key variables.

```{r Quick Histograms of players}
# select variables to be examined by histogram
...
##### **hero_id**
The variable 'hero_id' represents an integer that when linked to 'hero_names.csv' identifies which hero each player selected prior to each match.  This will be key in determining which heroes are best and which should be avoided when first starting out in the game. It is noted that 'hero_id' ranges from 'r min(players$hero_id)' to 'r max(players$hero_id)', however hero_id 0 is invalid and represents when a hero was not selected. This means these matches with hero_id 0 were inbalanced and should be removed from the data set.
...
Applying this to MatchDF has only removed one match from this data set since the duration limits previously applied to the data set has already filtered most of these matches, as would be expected. When teams are inbalanced it would be expected that the match duration would be outside the expected duration of a competitive match.  There are now 49677 matches in this data set.
...
This histogram shows quite a variance on the number of times each hero is selected every match and shows their popularity within the matches studied here. This will be imprtant when investigating the different win conditions.
...
The variables 'kills' and 'deaths' are player-vs-player (PvP) combat statistics collected per player which represent the number of enemy player kills scored per match and the number of deaths the player suffered by enemy hands per match. The number of assists per match represents the number of enemy player deaths each player significantly contributed to, but did not land the final hit on.  These variables will help to indicate how well each players' performance is relative to all the other player in the match. Further investigations could then link each player by their player_id assignment previously mentioned. These show normal distributions and there are no reasons to exclude any data based upon these variables.
...
##### **level**
The level variable idicates what level the player reach at match end. The distribution is fairly flat above 10. This variable will be helpful when comparing hero levels from winning and losing teams.
...
##### **leaver_status**
The leaver status variable ranges from 0 to 4, where 0 indicates matches where no one left and 1-4 indicate varying degrees of when players left during matches. The important point here is to only look at matches where no one left the match. As indicated by the histogram most of the matches did not have any leavers.
...
This shows there are 9593 players who are redcorded leaving the match at some point. These matches should be removed from the data set because even if only one player leaves this represents a loss of force by 20%. This gives the team with all players present an advantage over the other significantly and will skew all the variables recorded for both teams. It maybe interesting at a later date to compare these matches with leavers to those whithout to see how much this affects match outcomes.
...
For this study the following 26 variables from the orginal file 'players' will be used:
...
The hero_names.csv file was updated to include each heroes' roles and class as defined by dota2.gamepedia.com/Role [link](http://dota2.gamepedia.com/Role).  Each hero can have multiple roles but only one of three classes.  The three classes and theor descriptions are:
...
For more details on roles please see dota2.gamepedia.com/Role [link](http://dota2.gamepedia.com/Role). The data frame Hero_Names was exported as a csv file and modifed to include the class and role information in Excel. It now looks like this.

```{r Hero_Names Modified}
...
The 'CombinedDF' data frame has 420510 observations across 49 variables and represents the collection of all data that will be discussed in this report. This will be the source for all studies going forward in this project.
