...
Here we see Omniknight has the highest win percentage of `r max(Hero_Bar$Hero_Win_Pct)` and Shadow Demon has the lowest win percentage of `r min(Hero_Bar$Hero_Win_Pct)`. We will examine these two heroes as well to get an idea of how this

Lets look at the linear model as see what it tells us.
...
Model 'LM2' displays a very low P value for all concerned variables indicating that all these variables correlate with deaths showing a negative correlation to gold. We also see a significant improvement in the R-squared value to 0.16 indicating that adding deaths has improved the model. It is noted that in Dota 2 the amount of gold you lose upon death is tied to the hero's current level. Let's multiple level with deaths and see if this improves the model further.
...
Model 'LM3'  shows an improvement in R-squared to 0.23 however we also see that duration has a much higher probability of 0.21 that the null hypothesis for this variable is correct, that it does not hold a correlation in this model.

```{r duration vs level xy plot}
ggplot(CombinedDF, aes(x = duration, y = level, col = WL)) +
  geom_point(size = 0.2)

LvlDur_lm <- lm(level ~ duration, data=CombinedDF)
summary(LvlDur_lm)
