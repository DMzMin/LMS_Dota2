...
Let's see how these stack up when ordered by WLR and coloured by HN_Count.

```{r}
Hero_Bar$HN_Pct <- round((Hero_Bar$HN_Count/sum(Hero_Bar$HN_Count)*100), digits = 2)

ggplot(head(Hero_Bar, n = 40), aes(x = reorder(Name, -WLR), y = WLR, width = 0.8, fill = HN_Pct)) +
...
Let's take a look at heroes kill:death ratios (KDR) when they win and when they lose matches.
