...
Several variables were analysed in logistic regression models to infer their importance to winning Dota 2 matches. The following logistic regression model was made.
...
In this model it is seen that the variables with the largest impact on winning are the combat variables.  These are 'assists' which has a positive relationship to winning and 'deaths' which has a negative relationship to winning. 

It was determined in previous models that getting kills in matches did not significantly contribute to winning, but we see here that getting assists does positively impact winning. This means that for players to win they should contribute to fighting and help out their teammates but should not worry about getting that final blow. They should not over-extend themselves just to get a kill as this can expose themselves to unnecessary risks of dying. Avoiding death is very important to being successful in Dota 2.

### **Future Work**

The data frame 'CombinedDF' has a very large mix of diverse variables that could be used in many ways to explore the game Dota 2. For instance this study has only looked at Windranger, the most selected hero within this dataset. Windranger is a hero that is attributed to five different roles representing a 'jack-of-all-trades' type character, with slightly more losses than wins recorded. Future analysis should look at other heroes such a Shadow Fiend, the second most selected hero in this dataset. Shadow Fiend is associated with only two roles which are all about combat and has slightly more wins than losses recorded.

If we simply compare these two heroes' duration variable split by win loss we see the following.
...
These two plots quickly show how detailed and diverse this dataset really is and opens up possible avenues of thought for further exploration. For example it may be worth finding out which heroes fall into these two categories. This could influence team composition and strategy depending on those team compositions.

This leads into the diverse number of roles attributed to each hero. How do these play into winning and losing? One might imagine that a team consisting of only support heroes will struggle to win, as will teams with no support roles. How do different team compositions play out against each other? If one team is composed of only support heroes, then maybe a team without support heroes might be fine and win every time.

In this study the specific items that were bought throughout each match was excluded. However a detailed examination of this information might identify which items are more effective at improving a heroes chances of winning matches. It may also show that some items work better for different heroes.

For this study there were 33 separate columns dedicated to 'unit_order_' which were simply amalgamated into 'unit_order_total' for this study, however analysis of these individual variables may open interesting views on how to play, or how different players play. Unfortunately this dataset did not acquire very much information of individual players to determine play styles however if a more focused data extracting around players was conducted it maybe possible to identify different play styles for these players. This could lead to identifying best ways of playing Dota 2.

Further to this the dataset posted to Kaggle by devin contained many more csv files relating to these matches which could be brought into the study to further enrich future analysis. This study has only scratches the surface of the possible avenues of exploration here. It has been extremely interesting and I highly recommend this dataset for further analysis.
